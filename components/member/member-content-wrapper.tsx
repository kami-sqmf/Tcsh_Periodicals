...
import { getRolesProfiles, getTeams } from "@/utils/get-firestore";
...
const MembersContentWrapper = async ({ lang, className = "" }: { lang: LangCode; className?: string }) => {
  ...
  const {roles, profiles} = await getRolesProfiles(presentTeam[0] ? presentTeam[0].teamId : teams[0].teamId);
  return (
    <>
      ...
      <MembersContent lang={lang} teamsInfo={presentTeam.length > 0 ? presentTeam : teams} roles={roles} defaultProfiles={profiles} />
    </>
  )
}
