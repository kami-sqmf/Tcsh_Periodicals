...
import { isAdmin } from "../utils/getFirestore";
...
const NavLink = ({ nav, router, lang }: { nav: any, router: NextRouter, lang: langCode }) => (
    <Link href={nav.href}>
        <div className={`${(nav.href.split("/")[1] == router.pathname.split("/")[1]) && !nav.href.startsWith("http") ? "text-main2" : "text-main"} cursor-pointer py-1.5  font-medium flex flex-row items-center`}>
