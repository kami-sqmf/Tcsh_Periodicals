...
const EditorBlock = ({ data, onChange, editorId, serverData }: { data?: OutputData; onChange(val: OutputData): void; editorId: string; serverData: PostDocument }) => {
    ...
    <div id={editorId} className="w-full text-main prose-h2:text-lg prose-h2:font-medium my-6 flex flex-col">
      <div className="w-full">
        <div className="flex flex-row -ml-4 lg:ml-[54px]">
          ...
          <input id={"title"} defaultValue={serverData.title || ""} placeholder="Title" className="text-5xl font-bold bg-transparent focus:outline-none mb-2 flex-grow" onFocus={() => setTitleFocus(true)} onBlur={() => setTitleFocus(false)} />
        </div>
      </div>
    </div>
  )
