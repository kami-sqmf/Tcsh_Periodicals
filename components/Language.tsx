...
import { langCode, langIcons, langNames } from "../language/lang"
        ...
        <div className="flex flex-row items-center text-main2 px-2 justify-start cursor-pointer" onClick={() => setMenu(!menu)}>
            ...
            {menu && <div className="flex flex-row space-x-2">
                {Object.keys(langNames).filter((key) => key != lang).map((lang, key) => (
                    <Link href={`/${lang}/${router.pathname}`} key={key} className="flex flex-row scale-90 space-x-1 cursor-pointer">
