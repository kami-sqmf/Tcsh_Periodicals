...
const EbookModalWrapper = ({ lang, book, account, EbookBookCoverComponent }: { lang: LangCode; book: EBooks; account: Account | undefined; EbookBookCoverComponent: JSX.Element; }) => {
  const uploadConfirmRef = useRef<boolean>(false);
  const [modalOpen, setModalOpen] = useState<boolean>(false);
  const [modalSelect, setModalSelect] = useState<number>(0);
  const onBookClick = () => {
    if (account) {
      if (uploadConfirmRef.current) {
        setModalSelect(2);
        return setModalOpen(true);
      }
      if (!uploadConfirmRef.current) uploadConfirmRef.current = true;
      if (account.ownedBooks && account.ownedBooks.includes(book.files.bookId)) setModalSelect(2);
      else if (!book.locked) {
        addDoc(collection(db, "books", book.files.bookId, "license"), {
          voucher: false,
          used: true,
          usedTimestamp: serverTimestamp(),
          customer: doc(db, "accounts", account.uid),
          payment: "unlocked",
          createdTimestamp: serverTimestamp()
        });
        updateDoc(doc(db, "accounts", account.uid), { ownedBooks: arrayUnion(book.files.bookId) })
        setModalSelect(2);
  ...
  return (
    <>
      ...
      {modalSelect === 3 && <PurchaseModal lang={lang} modalOpen={modalOpen} setModalOpen={setModalOpen} bookId={book.files.bookId} userId={account!.uid} />}
    </>
  )
}
