...
const PrivacyPolicy = () => {
  ...
  const lang = locale ? locale as langCode : "zh";
  return (
    <PageWrapper lang={lang} page={Global.webMap.policy.child.privacy} withNavbar={true} operating={false}>
      <Breadcrumb args={[{ title: Global.webMap.policy.title(lang), href: "/policy", icon: RiAttachment2 }, { title: Global.webMap.policy.child.privacy.title(lang), href: "/policy/cookie", icon: Global.webMap.policy.child.privacy.nav.icon }]} />
      <div className="flex flex-col text-main mt-6 prose-h1:font-black prose-h1:text-4xl prose-h1:mb-4 prose-h2:font-bold prose-h2:text-xl prose-h2:mb-2 prose-h3:mb-4 prose-h3:font-medium prose-h3:text-base prose-h4:ml-4 prose-h4:font-normal prose-h4:text-base prose-h5:indent-4 prose-h5:ml-4 prose-h5:mb-3 prose-h5:font-normal prose-h5:text-base">
        ...
        <h4>- 以下內容只適用於情況 b.
          <h5>當您在本網站註冊帳號時即代表同意此服務條款。另外，本網站使用 Google第三方登入授權，即代表將會從 Google 獲取您的電子郵件、大頭貼以及用戶名稱等，同時本網站也會要求您提供以下基本資料：用戶名稱、自我介紹、Instagram 帳號、以及班級（假使電子信箱位置為慈中）。以上資料預設不公開，也不強制您提供，註冊後可在個人帳號頁面刪除，假使您投稿成功並發佈時，資料才會公開。</h5>
        </h4>
        ...
        <h5>但如果我們或我們的第三方分析服務提供商將匯總信息與您的個人數據結合或連接，以便它可以直接或間接識別您的身份，我們會將結合後的數據視為個人數據，並根據本隱私政策進行處理，請參閱我們的<Link href={`/${lang}/policy/cookie`} className="text-main2">Cookie 政策</Link>了解更多。</h5>
        <h2>六、與第三人共用個人資料之政策
