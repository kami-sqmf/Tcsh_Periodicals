            ...
            <div className="flex flex-col w-full px-12 sm:w-[350px] items-center rounded bg-white/80 py-2 mx-auto">
                ...
                <div className="my-5">
                    <form className="space-y-2 w-[256px]" onSubmit={submit} autoComplete="off">
                        ...
                        <p className="!mt-4 text-xs text-center text-gray-400"><span>{_t(lang).accounts.policy_service_upload}</span><Link href={`/${lang}/policy/`} className="text-gray-500 opacity-80 font-semibold">{_t(lang).accounts.policy_details}</Link></p>
                        <p className="!mt-4 text-xs text-center text-gray-400"><span>{_t(lang).accounts.policy_signup_agree}</span><Link href={`/${lang}/policy/`} className="text-gray-500 opacity-80 font-semibold">{_t(lang).accounts.policy_policy}</Link><span>{_t(lang).accounts.policy_and}</span><Link href={`/${lang}/policy/privacy`} className="text-gray-500 opacity-80 font-semibold">{_t(lang).accounts.policy_privacy}</Link><span>{_t(lang).accounts.policy_and}</span><Link href={`/${lang}/policy/cookie`} className="text-gray-500 opacity-80 font-semibold">{_t(lang).accounts.policy_cookie}</Link></p>
